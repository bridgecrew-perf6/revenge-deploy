...
def name(size, n):
    _l = []
    for i in range(size):
        _item = random_name(1, n)
        _l.append(_item)
    return "".join(_l)

def family_name():
    last_names = ['赵', '钱', '孙', '李', '周', '吴', '郑', '王', '冯', '陈', '褚', '卫',
                  ...
                  '和', '穆', '萧', '尹', '姚', '邵', '堪', '汪', '祁', '毛', '禹', '狄',
                  '米', '贝', '明', '臧', '计', '伏', '成', '戴', '谈', '宋', '茅', '庞',
                  '熊', '纪', '舒', '屈', '项', '祝', '董', '梁', '闫', '白', '蔡', '邓',
                  '杜', '段', '樊', '房', '风', '符', '福', '高', '古', '关', '郭', '胡',
                  '洪', '侯', '贾', '廖', '梁', '林', '刘', '龙', '陆', '卢', '牛', '裴',
                  '乔', '邱', '裘', '仇', '沙', '商', '尚', '师', '童', '万', '武', '肖',
                  '项', '徐', '羊', '阳', '易', '钟', '东方', '独孤', '慕容', '欧阳',
                  '司马', '西门', '尉迟', '长孙', '诸葛', '端木', '轩辕', '上官']
    times = 1
    return name(times, last_names)
...
def given_name():
    ...
    return name(times, first_names)


def full_name():
    _last = family_name()
    _first = given_name()
    return "{}{}".format(_last, _first)
